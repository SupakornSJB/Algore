...
public static class IntervalSchedulingTestHelper
{
        ...
        string saveFilePath)
    {
        ...
        foreach (var n in ns)
        {
            foreach (var _ in Enumerable.Range(0, trialPerConfig))
            {
                correctNPlots.Add(n);  
            }
        }
        ...
        foreach (var alpha in alphas)
        {
            ...
            correctAlphaPlots.Add(resultTimeForThisAlpha);
        }

        var plt = new Plot();
        foreach (var plot in correctAlphaPlots)
        {
            plt.Add.ScatterPoints(correctNPlots, plot);
        }
        ...
        string methodName)
    {
        ...
        for (var i = 0; i < ns.Length; i++)
        {
            ...
            {
                allValueOfThisN.AddRange(correctAlphaPlots[j][startIndex..endIndex].Select(x => x));
            }
